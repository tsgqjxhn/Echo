<template>
  <div class="history-page">
    ...
    <section v-if="activeTab === 'collected'" class="card-grid">
      <article
      ...
      >
        <img :src="character.avatar || defaultAvatar" :alt="character.name" class="card-avatar" />

        <div class="card-copy">
          <div class="card-header">
            <h2>{{ character.name }}</h2>
            <span class="pill">{{ character.category || '邀请角色' }}</span>
          </div>

          <p class="card-text">{{ character.description }}</p>

          <div class="meta-row">
            <span>{{ character.subCategory || '未细分' }}</span>
            <span>{{ formatDate(character.updatedAt) }}</span>
          </div>
        </div>

        <button type="button" class="card-action" @click.stop="goToChat(character.id)">
          继续对话
        </button>
      </article>
    </section>
...
type HistoryTab = 'collected' | 'liked' | 'chatted'
...
const tabMeta: Record<HistoryTab, { eyebrow: string; title: string; emptyEyebrow: string; emptyTitle: string; emptyCopy: string }> = {
  collected: {
    eyebrow: '收藏内容',
    title: '你保存下来的角色会出现在这里',
    emptyEyebrow: '暂无收藏',
    emptyTitle: '还没有收藏角色',
    emptyCopy: '回到主页挑选感兴趣的角色，收藏后就能在这里快速找到。'
  },
  liked: {
    eyebrow: '赞过内容',
...
const currentCount = computed(() =>
  activeTab.value === 'collected' ? filteredCollectedCharacters.value.length : filteredSessionCardsForTab.value.length
)
const isEmpty = computed(() => currentCount.value === 0)

const searchPlaceholder = computed(() =>
  activeTab.value === 'collected' ? '搜索收藏的角色或分类' : '搜索角色名或聊天内容'
)
...
function goToSession(session: SessionCard) {
  ...
  router.push(`/chat/${session.characterId}?sessionId=${session.representativeSessionId}`)
}
